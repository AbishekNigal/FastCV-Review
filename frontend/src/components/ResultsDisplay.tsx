...
import { TrendingUp, TrendingDown, Target, Users, ChevronDown, ChevronUp } from 'lucide-react';
    ...
    <div className="animate-fade-in" style={{ paddingBottom: '3rem' }}>
      ...
      <div style={{ display: 'grid', gap: '2rem' }}>
        {sortedResults.map((result, index) => {
          ...
          const allRisks = [...result.Gaps, ...result.Risk_Flags];
          const hasMore = result.Strengths.length > 3 || allRisks.length > 3;
          
          const displayedStrengths = isExpanded ? result.Strengths : result.Strengths.slice(0, 3);
          const displayedRisks = isExpanded ? allRisks : allRisks.slice(0, 3);
            ...
            <div key={index} className="glass-card" style={{ padding: 0, overflow: 'hidden' }}>
              ...
              {hasMore && (
                <div style={{ 
                  ...
                  textAlign: 'center',
                  background: isExpanded ? 'transparent' : 'linear-gradient(to top, white, transparent)',
                  ...
                  <button 
                    ...
                    style={{ 
                      ...
                      fontSize: '0.8rem', 
                      height: 'auto',
                      background: isExpanded ? 'rgba(0,0,0,0.05)' : 'var(--accent-primary)',
                      color: isExpanded ? 'var(--text-primary)' : 'white',
                      boxShadow: isExpanded ? 'none' : '0 10px 20px rgba(99, 102, 241, 0.2)'
                    }}
                  >
                    {isExpanded ? (
